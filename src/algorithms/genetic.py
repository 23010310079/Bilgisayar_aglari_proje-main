...
def csvden_parametreleri_oku(dosya_adi="input.csv"):
    parametreler = {}
...
class GenetikAlgoritma:
    def __init__(self, network, agirliklar,
                 pop_size=80, nesil_sayisi=100, mutasyon_orani=0.3):
...
if __name__ == "__main__":
    ...
    # 🔹 CSV’den input oku
    p = csvden_parametreleri_oku("input.csv")

    KAYNAK = int(p["kaynak"])
    HEDEF = int(p["hedef"])
    POP = int(p["populasyon"])
    NESIL = int(p["nesil"])
    MUT = float(p["mutasyon"])
    ...
    ga = GenetikAlgoritma(network, agirliklar, POP, NESIL, MUT)
